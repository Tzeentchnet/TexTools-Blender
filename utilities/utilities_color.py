...
color_id_layer_name = "TexTools_colorID"
temp_color_layer_name = "TexTools_temp"
gamma = 2.2
...
def set_active_color_layer(mesh, layer):
	...
	if hasattr(mesh, "color_attributes"):
		index = layers.find(layer.name) if hasattr(layers, "find") else -1
		if index >= 0:
			for attr_name in ("active_color_index", "active_index", "render_color_index"):
				if hasattr(layers, attr_name):
					try:
						setattr(layers, attr_name, index)
					except (AttributeError, TypeError):
						pass
	else:
		if hasattr(layer, "active"):
			layer.active = True
		if hasattr(layer, "active_render"):
			layer.active_render = True


def get_active_color_layer(mesh, fallback_name=None):
	if fallback_name:
		layer = get_color_layer(mesh, fallback_name)
		if layer:
			return layer
	...
	if hasattr(mesh, "color_attributes"):
		layer = getattr(layers, "active_color", None)
		if layer:
			return layer
		layer = getattr(layers, "active", None)
		if layer:
			return layer
		index = getattr(layers, "active_color_index", -1)
		if 0 <= index < len(layers):
			return layers[index]
	elif hasattr(layers, "active"):
		return layers.active

	return layers[0]


def ensure_color_layer(mesh, name, layer_type='BYTE_COLOR', domain='CORNER'):
	layers = get_color_layers(mesh)
	if layers is None:
		return None
	...
	if hasattr(mesh, "color_attributes"):
		if layer and getattr(layer, "domain", domain) != domain:
			layers.remove(layer)
			layer = None
		if not layer:
			layer = layers.new(name=name, type=layer_type, domain=domain)
	else:
		if not layer:
...
def remove_color_layer(mesh, name):
	layers = get_color_layers(mesh)
	if layers is None:
		return False
...
def set_polygon_color(layer, polygon, color):
	rgba = safe_color(color)
	for loop_index in polygon.loop_indices:
		layer.data[loop_index].color = rgba


def set_polygon_colors(mesh, layer, polygons, color):
	for polygon in polygons:
		set_polygon_color(layer, polygon, color)
	mesh.update()
	...
	if material:
		...
		for n in material.node_tree.nodes:
			if n.bl_idname == "ShaderNodeBsdfPrincipled":
				base_color = n.inputs.get("Base Color") or n.inputs[0]
