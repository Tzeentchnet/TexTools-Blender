...
def get_object_texture_image(obj):
	...
	for slot_mat in obj.material_slots:
		material = slot_mat.material
		if not material:
			continue

		# Check if material uses Nodes
		if getattr(material, 'node_tree', None):
			for node in material.node_tree.nodes:
				if node.bl_idname == 'ShaderNodeTexImage' and node.image:
					return node.image

		# Check legacy texture slots when running older Blender versions.
		for slot_tex in getattr(material, 'texture_paint_slots', ()) or ():
...
def image_resize(image, size_x, size_y):
	if image and image.source in {'FILE', 'GENERATED'}:
...
def get_tile_size(self, image, udim_tile):
	...
	if tile_name not in bpy.data.images:
		...
		base_tile = re.findall(r'\d{4}', base_image_location)[-1]
