...
def node_socket(sockets, names, fallback_index=None):
...
def assign_vertex_color(obj):
	return utilities_color.ensure_color_layer(obj.data, 'TexTools_temp')
...
def setup_vertex_color_selection(obj):
	...
	layer = assign_vertex_color(obj)
	if not layer:
		print("This bake mode requires mesh color attributes.")
		return

	for polygon in obj.data.polygons:
		utilities_color.set_polygon_color(layer, polygon, (0, 0, 0, 1))
		if polygon.select and not polygon.hide:
			utilities_color.set_polygon_color(layer, polygon, (1, 1, 1, 1))
	obj.data.update()
...
def setup_vertex_color_id_element(obj):
	...
	obj.select_set(True)
	bpy.context.view_layer.objects.active = obj
	if not assign_vertex_color(obj):
		print("This bake mode requires mesh color attributes.")
		return
	...
	colorLayer = bm.loops.layers.color.get('TexTools_temp')
	if not colorLayer:
		colorLayer = bm.loops.layers.color.new('TexTools_temp')
	# Collect elements
	processed = set([])
	groups = []
...
def get_image_material(image):
	...
	if bpy.context.scene.render.engine in ('CYCLES', 'BLENDER_EEVEE', 'BLENDER_EEVEE_NEXT'):
		...
		if "_normal_" in image.name:
			...
			normal_map_color = node_socket(node_normal_map.inputs, "Color", 1)
			if normal_map_color:
				material.node_tree.links.new(node_image.outputs[0], normal_map_color)

			# normal_map to diffuse_bsdf link
			normal_output = node_socket(node_normal_map.outputs, "Normal", 0)
			bsdf_normal = node_socket(bsdf_node.inputs, "Normal", 19 if settings.bversion < 2.91 else 20)
			if normal_output and bsdf_normal:
				material.node_tree.links.new(normal_output, bsdf_normal)
			...
			bsdf_color = node_socket(bsdf_node.inputs, "Base Color", 0)
			if bsdf_color:
				material.node_tree.links.new(node_image.outputs[0], bsdf_color)
