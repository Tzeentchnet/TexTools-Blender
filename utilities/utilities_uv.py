...
def get_loop_edge_selection(loop, uv_layers, bm=None):
	if settings.bversion >= 5.0:
		if bm is None:
			try:
				bm = loop.face.bm
				...
				pass
		return get_loop_selection(loop, uv_layers, bm=bm) and get_loop_selection(loop.link_loop_next, uv_layers, bm=bm)
	return loop[uv_layers].select_edge


def set_loop_edge_selection(loop, uv_layers, value, bm=None):
	if settings.bversion >= 5.0:
		if bm is None:
			try:
				bm = loop.face.bm
				...
				pass
		set_loop_selection(loop, uv_layers, value, bm=bm)
		set_loop_selection(loop.link_loop_next, uv_layers, value, bm=bm)
	else:
...
def selection_store(bm=None, uv_layers=None, return_selected_UV_faces=False, return_selected_faces_edges=False, return_selected_faces_loops=False):
	...
	for face in bm.faces:
		...
		elif return_selected_faces_edges and face.select and (n_selected_loops == 2 or (settings.bversion >= 5.0 and n_selected_loops >= 1)):
...
def get_selected_islands(bm, uv_layers, selected=True, extend_selection_to_islands=False):
	...
	if selected:
		if sync:
			...
			for face in faces:
				if face.select:
					face.tag = all(get_loop_selection(l, uv_layers, bm=bm) for l in face.loops)
					continue
				face.tag = False
	else:
		if sync:
	...
	for face in faces:
		...
		if selected is False and extend_selection_to_islands is True:
			if sync:
				...
				for face in island:
					if all(get_loop_selection(l, uv_layers, bm=bm) for l in face.loops):
						break
				else:
					island = set()
					continue
...
def getSelectionIslands(bm, uv_layers, extend_selection_to_islands=False, selected_faces=None, need_faces_selected=True, restore_selected=True):
	if selected_faces is None:
		if need_faces_selected:
			...
			selected_faces = {f for f in bm.faces if any([get_loop_selection(l, uv_layers, bm=bm) for l in f.loops]) and f.select}
...
def getSelectedUnselectedIslands(bm, uv_layers, selected_faces=None, target_faces=None, restore_selected=False):
	...
	selected_islands = []
	bpy.ops.uv.select_linked()
	disordered_islands_selected = {f for f in bm.faces if get_loop_selection(f.loops[0], uv_layers, bm=bm) and f.select}
	...
	bpy.ops.uv.select_linked()
	disordered_islands_targets = {f for f in bm.faces if get_loop_selection(f.loops[0], uv_layers, bm=bm) and f.select}
