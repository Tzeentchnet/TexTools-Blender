...
from ..utilities import utilities_bake as ub
...
def image_tile_add(number, width, height, is_float_32, color):
	kwargs = {
		'number': number,
		'width': width,
		'height': height,
	...
	prop_names = {prop.identifier for prop in bpy.ops.image.tile_add.get_rna_type().properties}
	if 'float_type' in prop_names:
		kwargs['float_type'] = is_float_32
	elif 'float' in prop_names:
		kwargs['float'] = is_float_32
	bpy.ops.image.tile_add(**kwargs)
...
class op(bpy.types.Operator):
	...
	bl_label = "Bake"
	bl_description = "Bake selected objects using the active TexTools bake mode"
	...
	def poll(cls, context):
		...
		if modes[bake_mode].setVColor or not modes[bake_mode].material:
			...
			def is_vc_ready(obj):
				if utilities_color.color_layer_count(obj.data) > 7:
...
def bake(self, mode, size, bake_force, sampling_scale, circular_report, color_report, selected, active, pre_selection_mode):
	...
	try:
		...
		for obj in previous_materials:
			...
			if material_loaded:
				if modes[mode].setVColor:
					utilities_color.remove_color_layer(obj.data, 'TexTools_temp')
...
def apply_composite(image_name, scene_name, size):
	...
	if scene_name in bpy.data.scenes:
		...
		path = paths.resource_path("compositing.blend", "Scene")
...
def setup_image(color_report, mode, name, width, height, tiles, material_load=False):
	...
	def resize(image):
		generated_width = getattr(image, 'generated_width', image.size[0])
		generated_height = getattr(image, 'generated_height', image.size[1])
		if image.size[0] != width or image.size[1] != height or generated_width != width or generated_height != height:
	...
	def image_create():
		...
		if tiles and settings.bversion >= 3.2:
			...
			for tile in tiles:
				image_tile_add(tile, width, height, is_float_32, bake_back_color)
...
def get_material(mode):
	...
	path = paths.resource_path("materials.blend", "Material")
	if "bevel" in mode or "thickness" in mode:
		path = paths.resource_path("materials_2.80.blend", "Material")
	# print("Get material {}\n{}".format(mode, path))
