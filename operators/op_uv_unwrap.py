...
from ..utilities.utilities_bbox import BBox
...
def main(self, axis):
	...
	for bm, uv_layer, sel_states, full_islands, edge_seam, pin_state, uv_coords, orient_uvs in groups:
		...
		for loop, sel_state in zip(loops, sel_states):
			utilities_uv.set_loop_selection(loop, uv_layer, sel_state[0], bm=bm)
			utilities_uv.set_loop_edge_selection(loop, uv_layer, sel_state[1])
