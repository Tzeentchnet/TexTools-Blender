		...
		if image:
			...
			for obj in objects:
				...
				if obj.modifiers:
					...
					obj.modifiers.active.node_group = nodegroup
					obj.modifiers.active.show_render = False
				move_modifier_to_index(obj, 'TT-material-override', len(obj.modifiers)-1)

			bpy.context.view_layer.objects.active = bpy.data.objects[preactiv_name]


def move_modifier_to_index(obj, modifier_name, index):
	modifier_index = obj.modifiers.find(modifier_name)
	if modifier_index < 0:
		return
	if hasattr(obj.modifiers, "move"):
		obj.modifiers.move(modifier_index, index)
	elif hasattr(bpy.ops.object, "modifier_move_to_index"):
		bpy.ops.object.modifier_move_to_index(modifier=modifier_name, index=index)



def get_override_nodegroup():
	if bpy.data.node_groups.get('TT-material-override') is None:
		path = paths.resource_path("materials_3.0.blend", "NodeTree")
